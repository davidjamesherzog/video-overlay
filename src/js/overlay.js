...
class Overlay {
  ...
  constructor(player, options) {
    ...
    if (this.startEvent_ === 'timeupdate') {
      this.player_.addEventListener('timeupdate', this.rewindListener_.bind(this));
    }
  ...
  createEl() {
    ...
    const background = options.showBackground ? 'video-overlay-background' : 'video-overlay-no-background';
    this.el_ = document.createElement('div');
    this.el_.className =  `video-overlay video-overlay-${options.align} ${options.class} ${background} video-hidden`;
  ...
  hide() {
    // need to hide here
    if (this.el_) {
      this.el_.classList.add('video-hidden');
    }
